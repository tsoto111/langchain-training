...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    return round(float(price) * (1 - discount_percentages.get(discount_tier, 0) / 100), 2)

# Change 2: Use a dictionary to map tool name to function
tools = {
...
Action Input: the input to the action, as a comma separated values
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Stop token prevents LLM from generating its own Observation - 
        ...
        # Change 6: Parsing tool calls from raw text with regex - fragile if LLM doesn't follow format.
        ...
        # Difference 7: History is one growing string re-sent every iteration (replaces messages.append).
